# BUILD — Filtering, restriction, and license policy

## 1. Implementation Scope

- Changed: `target/internal/policy`, `target/internal/diagnostics`, `target/internal/spdxcompat`.
- Covered: `--unknown`, `--onlyunknown`, `--exclude`, BSD alias, escaped comma, package whitelist/blacklist, private exclusion, `--failOn`, `--onlyAllow`, exact diagnostics/exit code `1`.

## 2. Build Steps

1. Implemented policy pipeline after sorted records.
2. Implemented exclusion parser/evaluator and bounded SPDX compatibility.
3. Implemented package restrictions, private deletion, and fail policy diagnostics.

## 3. Test Evidence

- Tests added: `internal/policy/policy_test.go`, `internal/diagnostics/diagnostics_test.go`.
...
- Full SPDX satisfaction beyond recorded MIT/ISC/BSD/Public Domain-style examples is bounded compatibility, not a full external-library parity claim.

## 5. Traceability

- F-005 FR-001–FR-027 map to `internal/policy/policy.go`, `internal/spdxcompat/spdxcompat.go`, and `internal/diagnostics/diagnostics.go`.
