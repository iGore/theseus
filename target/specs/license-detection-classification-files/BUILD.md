# BUILD — License detection, classification, and license files

## 1. Implementation Scope

- Changed: `target/internal/license`, `target/internal/spdxcompat`, `target/internal/records` license application seam.
- Covered: SPDX pass-through examples, ordered regex classifier, falsey/first-newline behavior, license-file precedence, README/package/file flow, licenseFile/licenseText/noticeFile fields.

## 2. Build Steps

1. Implemented bounded SPDX compatibility facade for recorded examples.
2. Implemented ordered classifier and file precedence detector.
3. Integrated package/readme/license-file application in record flattening.

## 3. Test Evidence

- Tests added: `internal/license/license_test.go` for classifier and file precedence.
...
- SPDX facade is intentionally bounded to documented examples rather than a full replacement for all transitive SPDX package behavior.
- `copyright` extraction is not fully source-fixtured.

## 5. Traceability

- F-004 FR-001–FR-011 map to `internal/license/license.go`, `internal/spdxcompat/spdxcompat.go`, and `internal/records/records.go`.
